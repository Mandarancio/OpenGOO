...
void Joint::status(){
    ...
    if (l<5.0 || l>20.0 || force>( a->isDragging() || b->isDragging()? 0.002 : 0.75) ) { //If the joint is too short or too long or the force is too much broke the joint
        a->destroyLink(b);
        b->destroyLink(a);
        emit destroyJoint(this);
    }
